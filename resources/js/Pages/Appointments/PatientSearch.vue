...
<template>
  <div class="patient-search-wrapper">
    <div class="row">
      <div class="col-md-9">
        <input :value="searchQuery" @input="e => handleSearch(e.target.value)" type="text"
          class="form-control form-control-lg rounded-lg mb-2" placeholder="Search patients by name or phone..."
          @focus="showDropdown = searchQuery && searchQuery.length >= 2" />
      </div>

      <div class="col-md-3">
        <button type="button" class="btn btn-primary rounded-pill" @click="openModal()">
          Add New Patient
        </button>
      </div>
    </div>
          ...
          <div v-for="patient in patients" :key="patient.id" class="patient-item" @click="selectPatient(patient)">
            <div class="patient-info border-0">
              <div class="patient-name">
                <h6 class="fw-bold text-sm">{{ patient.first_name }} {{ patient.last_name }}</h6>
